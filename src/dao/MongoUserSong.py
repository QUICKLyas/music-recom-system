...
class UserSongRecom (object):
    ...
    # 第一步，通过MongoTagUser生成一个相似的用户集
    def makeSimilarityUsersById(self) -> dict:
        list_users = []
        # 通过self.mtu生成对应的数据
        list_users = self.mtu.makeTagUserRateDataPandas()
        return list_users

    # 第二步，根据上面的用户，构成一个新的关于User 和 Song 的用户集
    def makeSimilaritySongByUsers(self):
        matrix_user_song = []
        return matrix_user_song
    # ps 用户集的选取，应该根据相似度排序后，去其中相似度较高的用户，然后随机选取一定数量的用户，根据这些用户生成用户集
    # 第三步，根据上面的新的用户集与歌曲集的结合，获取相似歌曲，推荐给用户，
